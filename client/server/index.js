...
const app = express();
const PORT = process.env.BOOKING_SERVER_PORT || 4000;

const allowedOrigins = process.env.BOOKING_ALLOWED_ORIGINS
  ? process.env.BOOKING_ALLOWED_ORIGINS.split(",").map((origin) => origin.trim())
  : "*";
...
const ensureTransporter = () => {
  ...
  const pass = process.env.BOOKING_SMTP_PASSWORD;

  if (!user || !pass) {
    throw new Error("Missing BOOKING_SMTP_USER or BOOKING_SMTP_PASSWORD in environment.");
  }
